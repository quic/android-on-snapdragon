...
public class ProSightEncode {
    private final static String TAG = "PROSIGHT_ENCODER";
    ...
    public ProSightEncode(MainActivity activity) {
        mainActivity = activity;
    }

    void runProSightEncode(String fileInput, String fileOut, int Bitrate, int FRAME_INTERVAL)
            throws Exception {
    ...
    private Boolean SetProSightEncode() {
        boolean value = QMediaCodecCapabilities.isProSightSupported(mEncodec);
        if(!value) {
            Log.e(TAG, "isProSightSupported: " + value);
            mainActivity.updateUI("\n ProSight is not Supported");
            return false;
        }
        EncoderFormat = QMediaCodecCapabilities.enableProSight(EncoderFormat);
        return true;
    }

    private boolean createFormat() throws IOException {
        ...
        for (int i = 0; i < extractor.getTrackCount(); i++) {
            ...
            if (mime.startsWith("video/")) {
                Log.i(TAG, "MediaExtractor mime : " +  mime);
        ...
        EncoderFormat.setInteger(MediaFormat.KEY_PROFILE, MediaCodecInfo.CodecProfileLevel.HEVCProfileMain10);
        EncoderFormat.setInteger(MediaFormat.KEY_LEVEL, MediaCodecInfo.CodecProfileLevel.HEVCMainTierLevel51);
        return true;
    }
    ...
    private void Decodeinput() {
        ...
        D_InputFrame++;
        Log.v(TAG, D_InputFrame + " D_InputFrame Time: " + Decodecinfo.presentationTimeUs);
        if (sampleSize < 0) {
